...
export abstract class BaseService<T> {
    ...
    this.topicSubscription = this.rxStompService
      ...
      .subscribe((message: Message) => {
        ...
        try {
          ...
          if (messageData.name === this.apiUrl && Array.isArray(messageData.objects)) {
  ...
  gets(): Observable<T[]> {
    ...
    const objectsObservable = this.apiService.request<T[]>('get', this.apiUrl);
  ...
  private isInCache(name: string, idToExclude: number | null = null): boolean {
    const isInCache = this.cache?.some(
      (cache) =>
        this.getItemName(cache).toLowerCase() === name.toLowerCase() && this.getItemId(cache) !== idToExclude
    );
    if (isInCache) {
  ...
  add(newObject: T): Observable<T> {
    if (this.cache) {
      if (this.isInCache(this.getItemName(newObject))) {
        return of();
      }
    }
    return this.apiService.request<T>('post', this.apiUrl, newObject).pipe(
      tap((added: T) => {
        ...
        localStorage.setItem(this.cacheKey, JSON.stringify(this.cache));
        this.onSendMessage(this.apiUrl);
      })
    );
  }
  ...
  update(updatedObject: T): Observable<any> {
    if (this.cache) {
      if (this.isInCache(this.getItemName(updatedObject), this.getItemId(updatedObject))) {
        return of();
      }
    }
  ...
  searchByName(term: string): Observable<T[]> {
    ...
    return this.apiService.request('get', this.apiUrl).pipe(
      tap({
        next: (response: any) => {
